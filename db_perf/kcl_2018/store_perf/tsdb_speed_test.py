	...
	def open(self) :
		...
		while bConti :
			try :
				...
				self.m_dConn = InfluxDBClient('127.0.0.1', 8086, '', '', 'store_speed')
				bConti = False
			except Exception:
...
def update():
	...
	for i in range(1,11):
		reader = csv.reader(open('cv_data_50k.csv'))
		for row in reader:
			key = int(row[0])
	...
	for list_idx in range(0,10):
		...
		tmp_list = []
		tmp_dict = {}
		for idx in range(0, int(DATA_CNT/CHUNK_SIZE)):
		...
		for insert_idx in range(0, int(DATA_CNT/CHUNK_SIZE)):
		...
		print(" Avg. run-time of 50,000 data insertion=", ctime, "seconds")
		print
