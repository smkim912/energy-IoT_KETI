...
SRC_TABLE = 'Z_BACKUP_IF_ONP2'
MERGE_TABLE = 'DISP_ONP'
COL_NAME_PREFIX = 'ONP'
CREATE_TABLE_QUERY = "create table DISP_ONP(TIMESTAMP datetime)"
START_DATE = '2018-07-26 03:44:49'
#START_TS = (1532530800-1) * 1000000000 #equal to START_DATE
DEFAULT_QUERY_RANGE = 600 #seconds
QUERY_INTERVAL = 5 #seconds
MAX_SUSP_TIME = 60*60*24*30 #1-month
END_DATE = '2018-10-16 23:59:58' #old only
...
tagmap = onp_tagmap.const_onp_tagmap
...
def same_time_merge(_list):
	...
	while True:
		...
		for m in range(0, len(merge_list)):
			...
			sql += tagmap[merge_list[m]['TAG_ID']]
		...
		if ins_db.execute(sql) is 0:
			...
			for l in range(0, len(merge_list)):
				...
				alt_sql += tagmap[merge_list[l]['TAG_ID']]
				alt_sql += " long)"
